# ------------
# simulation 1-2: continuous outcome, uniform sampling, MAR
...
for(iter in 1: nrow(p.grid)){
  ...
  tryCatch({
    ...
    tmle.fit.cl <- tmle(Y = cl_data$RY, A = cl_data$A, W = cl_data[,c('R_X1', 'RX1', 'R_X2', 'RX2', "R_C1", "RC1", "M")], Q.SL.library = SLmethods, g.SL.library = SLmethods, V.Q=2, V.g=2)
  ...
  }, error = function(e) {results[(2 * iter - 1):(2 * iter)] <- c(NA, NA)})
  
}


write.csv(results, file = paste0("temp-results/sim1-2-results_", param, ".csv"), row.names = FALSE)
...
files <- list.files(pattern = 'sim1-2-results_.*\\.csv')
results <- do.call(rbind, lapply(files, function(j) {t(read.csv(j))}))
write.csv(results, '../sim1-2-results.csv', row.names = FALSE)

# summarizing results ---------
results <- read.csv("TMLE/sim1-2-tmle-results.csv")
delta <- 5
est_index <- c(1,3,5,7)
se_index <- c(2,4,6,8)
