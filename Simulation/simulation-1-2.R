# ------------
# simulation 1: continuous outcome, uniform sampling, MAR
...
results <- foreach(iter = 1:sim_size, .combine = cbind, .packages = package_list) %dopar% {
  ...
  tryCatch({
    ...
    ipw <-geeDREstimation(formula=RY~A, nameY = "RY", nameMISS= "R_Y", nameTRT = "A",
                          ...
                          sandwich.nuisance = T,
                          model.weights=I(R_Y==1)~A + R_X1 + RX1 + R_X2 + RX2 + R_C1 + RC1 + M)
    ...
    aug_gee <- geeDREstimation(formula=RY~A, nameY = "RY", nameMISS= "R_Y", nameTRT = "A",
                               ...
                               model.weights=I(R_Y==1)~A + R_X1 + RX1 + R_X2 + RX2 + R_C1 + RC1 + M,
                               model.augmentation.trt= RY~ R_X1 + RX1 + R_X2 + RX2 + R_C1 + RC1 + M,
                               model.augmentation.ctrl= RY~ R_X1 + RX1 + R_X2 + RX2 + R_C1 + RC1 + M
    )
...
tictoc::toc()
stopCluster(cl)
saveRDS(results, paste0("simulation-1-2-m=",m,"-mp=",mp,".rds"))

# summarizing results
results <- readRDS(paste0("simulation-1-2-m=",m,"-mp=",mp,".rds"))
